...
namespace Minimalist.Bar
{
    public class LabelManagerBhv : UIElementBhv
    {
        ...
        public void UpdateLabel(float amount, float minimum, float maximum, LabelDisplayFormat format)
        {
            ...
            switch (format)
            {
                case LabelDisplayFormat.CurrentAmount:

                    text = amount.ToString("f0");

                    break;

                case LabelDisplayFormat.CurrentOverMaximumAmount:

                    text = leadingSpaces + amount.ToString("f0") + "/" + maximum.ToString("f0");

                    break;

                case LabelDisplayFormat.CurrentFillPercentage:

                    text = (amount / (maximum - minimum) * 100f).ToString("f0") + "%";
...
public enum LabelDisplayOptions
{
    Always,
    IfFillAmountIsMoreThan,
    IfFillAmountIsLessThan,
    IfFillAmountIsBetween
}

public enum LabelDisplayFormat
{
    CurrentAmount,
    CurrentOverMaximumAmount,
