...
namespace Minimalist.Bar
{
    ...
    public class BarBhv : QuantitySubscriber
    {
        ...
        [SerializeField] private LabelDisplayOptions _labelDisplayOptions = LabelDisplayOptions.Always;
        [SerializeField] private LabelDisplayFormat _labelDisplayFormat = LabelDisplayFormat.CurrentOverMaximumAmount;
        [SerializeField, Range(0, 1)] private float _labelMinFillAmount = -Mathf.Infinity;
        [SerializeField, Range(0, 1)] private float _labelMaxFillAmount = +Mathf.Infinity;
        ...
        {
            bool displayOptionsEffect = true;

            switch (_labelDisplayOptions)
            {
                case LabelDisplayOptions.IfFillAmountIsMoreThan:

                    displayOptionsEffect = Quantity.FillAmount > _labelMinFillAmount;

                    break;

                case LabelDisplayOptions.IfFillAmountIsLessThan:

                    displayOptionsEffect = Quantity.FillAmount < _labelMaxFillAmount;

                    break;

                case LabelDisplayOptions.IfFillAmountIsBetween:

                    displayOptionsEffect = Quantity.FillAmount > _labelMinFillAmount && Quantity.FillAmount < _labelMaxFillAmount;

                    break;
            }

            LabelManager.UpdateLabel(Quantity.Amount, Quantity.MinimumAmount, Quantity.MaximumAmount, _labelDisplayFormat); 

            LabelManager.IsActive = _displayLabel && displayOptionsEffect;
        ...
        private void UpdateBar()
        {
            ...
            LabelManager.IsActive = _displayLabel && (Quantity.FillAmount > _labelMinFillAmount && Quantity.FillAmount < _labelMaxFillAmount);
