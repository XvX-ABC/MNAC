...
namespace Tests.Characters
{
    internal class DeathStateHelper
    {
        ...
        public DeathPanel DeathPanel { get => state.DeathPanel; set => state.DeathPanel = value; }
        public MainPlane MainPanel { get => state.MainPanel; set => state.MainPanel = value; }
    ...
    internal class DeathState : CharacterBehaviourStateBase
    {
        ...
        DeathPanel _deathPanel;
        MainPlane _mainPanel;
        public LocomotionCore LocomotionCore
        {
            get => _locomotionCore;
            set
        ...
        public DeathPanel DeathPanel { get => _deathPanel; set => _deathPanel = value; }
        public MainPlane MainPanel { get => _mainPanel; set => _mainPanel = value; }
        ...
        public DeathState(GameObject obj, Action<GameObject> startAction, Action<GameObject> endAction, float duration = 0, bool enabled = true) : base("death", duration, enabled)
        {
            ...
            _diedLocomotion = new();
            this.timeline.AddPointEvent(0, _ =>
            {
                _startAction?.Invoke(_obj);
                if (_deathPanel != null)
                    _deathPanel.Hide = false;
            });
        ...
        void DoDied(TimelineContext _)
        {
            ...
            if (_deathPanel != null)
            {
                _deathPanel.Hide = true;
                if (_mainPanel != null)
                {
                    _mainPanel.Hide = false;
                    _mainPanel.EnableContinueButton = false;
                }
            }
