...
{
    [CustomEditor(typeof(X0_MultiMissileLauncherDefines))]
    public class X0_MultiMissileLauncherDefinesEditor : UnityEditor.Editor
    {
        ...
        private void OnEnable()
        {
            ...
            var d = target as X0_MultiMissileLauncherDefines;
            _durations = (Durations)typeof(X0_MultiMissileLauncherDefines).GetField("_durations", BindingFlags.NonPublic | BindingFlags.Instance).GetValue(d);
            _ammo = serializedObject.FindProperty("_ammo");
            _mountPoints = serializedObject.FindProperty("_mountPoints");
        }
