...
namespace Tests.Weapons.Projectiles
{
    public class Bullet_New : MonoBehaviour
    {
        ...
        private void FixedUpdate()
        {
            ...
            var ray = new Ray(_rbody.position, transform.forward);
    ...
    public class Bullet : MonoBehaviour, IBullet
    {
        ...
        private void Awake()
        {
            _definition = GetComponent<IProjectileDefinitions>() ?? throw new ComponentCantFindException(gameObject, typeof(IProjectileDefinitions));
        ...
        void Start()
        {
            if (_effects != null)
            {
                ...
                {
                    _disableAction?.Invoke(this, gameObject);
                    enabled = false;
                });
            }
        }
        ...
        {
            Debug.Log(MethodBase.GetCurrentMethod().Name + "," + gameObject.name);
